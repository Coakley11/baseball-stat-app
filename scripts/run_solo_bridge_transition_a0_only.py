...
def _classify_a0(
    *,
    ...
    if provenance_error in ("provenance_empty", "provenance_unreadable"):
        return "INVALID", f"token_provenance_unreadable:{provenance_error}"
...
def run_a0(page, ws_frames: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    on_change_once = len(on_change_events) == 1
    session_has_token = any(
        e.get("source") == "on_change_callback" and str(e.get("actual_raw") or "") == expected for e in provenance
    ) or int(probe.get("valid_expiration_count") or 0) >= 1
    ...
    if isinstance(mutation_log, list) and mutation_log:
    ...
    return {
        ...
        "on_change_matching_token_count": len(on_change_events),
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "runner_commit_note": "2b437fa_tightened_consecutive_room_loss",
