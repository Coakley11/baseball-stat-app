...
def _button_visible(page, button_label: str, *, timeout_ms: int) -> bool:
    fr = _app_frame(page)
    try:
        loc = fr.get_by_role("button", name=button_label, exact=True)
        if loc.count() == 0:
            loc = page.get_by_role("button", name=button_label, exact=True)
        loc.first.wait_for(state="visible", timeout=timeout_ms)
        return loc.first.is_visible() and loc.first.is_enabled()
    except Exception:
        return False
...
def _probe_expander_label(page, expander_label: str) -> dict[str, Any]:
    merged: dict[str, Any] = {"found": False}
    for fr in page.frames:
        if "/~/" not in str(fr.url or "") and fr != page.main_frame:
            continue
        try:
            row = fr.evaluate(_FIND_EXPANDER_JS, expander_label) or {}
        except Exception as exc:
            row = {"error": str(exc)[:160]}
        if isinstance(row, dict) and row.get("found"):
            merged = dict(row)
...
def open_streamlit_expander_by_label(
    page,
    *,
    expander_label: str,
    visible_button_label: str,
    wait_after_open_ms: int = 1500,
) -> dict[str, Any]:
    label = str(expander_label or "").strip()
    visible = str(visible_button_label or "").strip()
    out: dict[str, Any] = {
        "label": label,
        "visible_button_label": visible,
    ...
    if not out["matrix_expander_initially_open"]:
        ...
        fr = _app_frame(page)
        try:
            row = fr.evaluate(_CLICK_SUMMARY_JS, label) or {}
        ...
        if not clicked:
            try:
                loc = fr.locator("details").filter(has_text=label)
                if loc.count() == 0:
                    loc = page.locator("details").filter(has_text=label)
    ...
    probe_after = _probe_expander_label(page, label)
    ...
    out["s0_visible_after_open"] = _button_visible(page, visible, timeout_ms=8000)
...
def open_fragment_identity_matrix_expander(page, *, wait_after_open_ms: int = 1500) -> dict[str, Any]:
    ...
    return open_streamlit_expander_by_label(
        page,
        expander_label=MATRIX_EXPANDER_LABEL,
        visible_button_label=S0_BUTTON_LABEL,
