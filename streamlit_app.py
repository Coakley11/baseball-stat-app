...
PAGE_GUIDES = {
    ...
    "Comparison Tool": {
        "purpose": "Compare up to three players side by side using stats, trends, and optional significance tests.",
        "when": "When you are choosing between a few names and want the same view for each.",
        "outputs": "Year-by-year table, career totals, trend charts, and category-style comparisons.",
    },
    "Trend Value": {
    ...
    "Fantasy Sleepers & Busts": {
        "purpose": "Find undervalued players, risky market picks, and names where your projections differ from ADP.",
        "when": "During draft prep or on draft day when you want buy-low and avoid-overpay lists.",
        "outputs": "Sleeper and bust tables, market-vs-model chart, and plain-language reasons.",
    },
    "Draft Room Simulator": {
...
def _prepare_historical_scatter_data(
    ...
    *,
    ...
    plot_df["Year"] = safe_numeric_series(plot_df, "yearID", np.nan)
...
def load_fantasypros_market_data():
    ...
    for col in ["FantasyPros Rank", "Expert Avg Rank", "Expert Std Dev", "ADP", "ADP Rank"]:
        ...
        if dup in market.columns:
            market[col] = safe_numeric_series(market, col, np.nan).fillna(pd.to_numeric(market[dup], errors="coerce"))
            market = market.drop(columns=[dup])
    market["Market Rank"] = safe_numeric_series(market, "ADP Rank", np.nan)
    market["Market Rank"] = market["Market Rank"].fillna(safe_numeric_series(market, "FantasyPros Rank", np.nan))
...
def make_fantasy_market_reason(row, kind="sleeper"):
    """Plain-language sleeper/bust note from model vs ADP gap and projected stat shape."""
    ...
    if pd.notna(proj_hr) and proj_hr >= 25:
        stat_bits.append(f"~{fmt_count_1(proj_hr)} HR projected")
    if pd.notna(proj_rbi) and proj_rbi >= 80:
        stat_bits.append(f"~{fmt_count_1(proj_rbi)} RBI projected")
    if pd.notna(proj_sb) and proj_sb >= 15:
        stat_bits.append(f"~{fmt_count_1(proj_sb)} SB projected")
    if pd.notna(proj_ops) and proj_ops >= 0.820:
        stat_bits.append(f"{fmt_rate_3(proj_ops)} OPS projected")
    stat_line = (" Stats: " + ", ".join(stat_bits[:3]) + ".") if stat_bits else ""

    if kind == "sleeper":
        if pd.notna(edge) and edge > 0:
            gap = (
                f"Our model ranks him **{fmt_int(model_rank)}** vs ADP **{fmt_int(market_rank)}** "
                f"(about **{fmt_int(edge)}** spots higher on our board)."
            )
        elif pd.notna(edge):
            gap = "Model and ADP are close — treat as a watch name, not a clear discount."
        else:
            gap = "No ADP match; leaning on our projection only."
        return f"**Sleeper:** {player}. {gap}{stat_line}"

    if pd.notna(edge) and edge < 0:
        gap = (
            f"ADP **{fmt_int(market_rank)}** is ahead of our model **{fmt_int(model_rank)}** "
            f"(about **{fmt_int(abs(edge))}** spots)."
        )
    elif pd.notna(edge):
        gap = "Model is not much lower than ADP — only a mild overpay flag."
    else:
...
def _store_ml_base_pack(base_sig, pack: dict, *, clear_tuning_cache: bool = True):
    st.session_state["_ml_base_sig"] = base_sig
    st.session_state["_ml_base_pack"] = pack
    st.session_state["_ml_proj_training_df"] = pack.get("ml_training_df", pd.DataFrame())
    st.session_state["_ml_proj_feature_cols"] = pack.get("ml_feature_cols", [])
    st.session_state["_ml_proj_models"] = pack.get("ml_models", {})
    st.session_state["_ml_proj_current_rows"] = pack.get("current_rows", pd.DataFrame())
    st.session_state["_ml_proj_base_pred_df"] = pack.get("base_pred_df", pd.DataFrame())
    if clear_tuning_cache:
        st.session_state.pop("_ml_tuning_cache", None)


def _ensure_ml_base_pack_precomputed(base_pack, yearly_source, ml_lookback, year_pool_sig, refresh_token):
    """Fill missing precompute (anchors/comps index) without clearing tuning cache."""
    if not isinstance(base_pack, dict) or base_pack.get("base_pred_df") is None:
        return base_pack
    ready = (
        base_pack.get("baselines") is not None
        and base_pack.get("age_curve_df") is not None
        and isinstance(base_pack.get("anchor_pool"), pd.DataFrame)
        and base_pack.get("similarity_index") is not None
    )
    if ready:
        return base_pack
...
def _ml_training_companion_rows(ml_training_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "predict_year" in ml_training_df.columns:
        return (
            ml_training_df.sort_values("predict_year")
            .groupby("playerID", as_index=False)
            .tail(1)
            .reset_index(drop=True)
        )
    return ml_training_df.drop_duplicates(subset=["playerID"], keep="last").reset_index(drop=True)


def _clear_ml_tuning_cache():
    st.session_state.pop("_ml_tuning_cache", None)
...
def _clear_ml_data_caches(*, clear_disk: bool = False):
    ...
    for fn in (
        ...
        build_ml_stabilized_anchor_pool,
...
def _ml_tuning_run_signature(reg, age_s, comp_w, k_nei, min_ab, projection_style="Balanced"):
    """Adjustment/calibration signature (fast re-run without retraining)."""
    return (
        float(reg),
        float(age_s),
        float(comp_w),
...
def _ml_projection_run_signature(yl_df, lookback, min_games, max_players, reg, age_s, comp_w, k_nei, min_ab, projection_style="Balanced"):
    """Full pipeline signature for session cache."""
    return _ml_base_run_signature(yl_df, lookback, min_games, max_players) + _ml_tuning_run_signature(
        reg, age_s, comp_w, k_nei, min_ab, projection_style
    )


def _nearest_age_adjustment_series(ages, age_curve_df, stat):
    """Nearest-bucket age adjustment for one stat (no per-row apply)."""
    out = pd.Series(0.0, index=ages.index, dtype=float)
    if age_curve_df.empty or ages.empty:
        return out
    stat_curve = age_curve_df.loc[age_curve_df["Stat"] == stat, ["Age", "Age Adjustment"]].sort_values("Age")
    if stat_curve.empty:
        return out
    age_vals = stat_curve["Age"].to_numpy(dtype=int)
    adj_vals = pd.to_numeric(stat_curve["Age Adjustment"], errors="coerce").fillna(0).to_numpy()
    rounded = pd.to_numeric(ages, errors="coerce").round()
    valid = rounded.notna()
    if valid.any():
        a_int = rounded[valid].astype(int).to_numpy()
        nearest_idx = np.abs(age_vals[:, None] - a_int[None, :]).argmin(axis=0)
        out.loc[valid] = adj_vals[nearest_idx]
    return out


def _empty_ml_pipeline_status():
...
def _run_ml_base_training_phase(
    *,
    yearly_source,
    ml_lookback,
    ml_min_games,
...
def apply_ml_stat_blend(
    ...
    *,
    ...
    """Vectorized RF + comp + age + regression blend (no similarity rebuild)."""
    if pred_df.empty:
        return pred_df
    ...
    for stat in target_stats:
        ...
        if "age_entering_year" in adjusted.columns:
            age_adj = _nearest_age_adjustment_series(adjusted["age_entering_year"], age_curve_df, stat)
...
def _get_or_build_ml_comp_df(base_pack, current_rows, ml_training_df, feature_cols, k_neighbors, refresh_token):
    """Session-scoped comp cache keyed by k_neighbors (BallTree reused via similarity_index)."""
    del refresh_token
    k_key = int(k_neighbors)
    comp_cache = base_pack.setdefault("comp_cache", {})
    if k_key in comp_cache and isinstance(comp_cache[k_key], pd.DataFrame):
    ...
    if comp_df.empty:
        ...
        comp_df = build_similar_player_predictions(
            ...
            train_comp,
            tuple(feature_cols),
            tuple(ML_TARGET_STATS),
            k_neighbors=k_key,
            refresh_token=0,
        )
    comp_cache[k_key] = comp_df
    return comp_df


def _finalize_ml_base_pack(base_pack, yearly_source, ml_lookback, year_pool_sig, refresh_token):
    """Precompute reusable similarity + anchor pool after RF training (amortizes slider reruns)."""
    if not isinstance(base_pack, dict) or base_pack.get("base_pred_df") is None:
        return base_pack
    timing = dict(base_pack.get("timing") or {})
    ml_training_df = base_pack.get("ml_training_df", pd.DataFrame())
    current_rows = base_pack.get("current_rows", pd.DataFrame())
    feature_cols = base_pack.get("ml_feature_cols", [])
    if ml_training_df.empty or current_rows.empty or not feature_cols:
        return base_pack
    base_pack["train_companions"] = _ml_training_companion_rows(ml_training_df)
    base_pack["age_curve_df"] = get_age_curve_adjustments(ml_training_df, tuple(ML_TARGET_STATS))
    base_pack["baselines"] = get_target_baselines(ml_training_df, tuple(ML_TARGET_STATS))
    t0 = time.perf_counter()
    base_pack["similarity_index"] = _build_ml_similarity_index(
        base_pack["train_companions"],
        tuple(feature_cols),
    )
    comp_cache = base_pack.setdefault("comp_cache", {})
    for _warm_k in (10, 25):
        comp_cache[_warm_k] = _query_ml_similarity_comps(
            base_pack["similarity_index"],
            current_rows,
            k_neighbors=_warm_k,
        )
    timing["precompute_similarity_sec"] = round(time.perf_counter() - t0, 3)
    t1 = time.perf_counter()
    base_pred = base_pack.get("base_pred_df", pd.DataFrame())
    if not base_pred.empty and "playerID" in base_pred.columns:
        anchor_ids = tuple(sorted(str(x) for x in base_pred["playerID"].astype(str).unique()))
        base_pack["anchor_pool"] = build_stabilized_projection_anchors(
            yearly_source,
            anchor_ids,
            int(ml_lookback),
        )
    else:
        base_pack["anchor_pool"] = pd.DataFrame()
    timing["precompute_anchors_sec"] = round(time.perf_counter() - t1, 3)
    base_pack["timing"] = timing
    return base_pack


def _run_ml_tuning_fast(
    *,
    yearly_source,
    ml_lookback,
    ml_min_ab,
    base_pack,
    effective_regression_strength,
    effective_age_strength,
    effective_comp_weight,
    k_neighbors,
    refresh_token,
):
    """Fast slider/tuning path: reuse RF output + cached comps/anchors."""
    from datetime import datetime

    status = _empty_ml_pipeline_status()
    status["ml_training_df"] = base_pack.get("ml_training_df", pd.DataFrame())
    status["ml_feature_cols"] = base_pack.get("ml_feature_cols", [])
    status["ml_models"] = base_pack.get("ml_models", {})
    status["current_rows"] = base_pack.get("current_rows", pd.DataFrame())
    status["base_pred_df"] = base_pack.get("base_pred_df", pd.DataFrame())
    ...
    try:
        ...
        if base_f.empty:
            status["message"] = (
                f"No players met **Minimum Recent AB** ({int(ml_min_ab)}). "
        ...
        status["timing"]["total_sec"] = status["runtime_sec"]
        status["timing"]["tuning_fast"] = True
        status["message"] = (
            f"Updated {status['player_count']:,} projections in {status['runtime_sec']:.1f} seconds "
            "(tuning-only; models not retrained)."
        )
        return status
...
def _run_ml_adjustment_and_calibration(
    *,
    yearly_source,
    ml_lookback,
    ml_min_ab,
    ...
    """Apply similarity/aging/regression + stabilized calibration (no RF retrain)."""
    if (
        isinstance(base_pack, dict)
        and base_pack.get("anchor_pool") is not None
        and base_pack.get("baselines") is not None
    ):
        fast = _run_ml_tuning_fast(
            yearly_source=yearly_source,
            ml_lookback=ml_lookback,
            ml_min_ab=ml_min_ab,
            base_pack=base_pack,
        ...
        if not fast.get("ok"):
            fb = fast.get("fallback") or "tuning_failed"
            return None, fast.get("timing") or {}, fb, fast.get("message")
        return {
            "pred_df": fast["pred_df"],
            "age_curve_df": fast.get("age_curve_df", pd.DataFrame()),
            "comp_df": fast.get("comp_df", pd.DataFrame()),
            "player_count": fast["player_count"],
        }, fast.get("timing") or {}, None, None
    ...
    timing["adjustment_sec"] = round(time.perf_counter() - t1, 3)
    if pred_df.empty:
        return None, timing, "adjustment_empty", "Projection adjustment returned no rows. Try different tuning sliders."
    ...
    if not anchors.empty:
        pred_df = proj_cal.apply_ml_final_output_calibration(pred_df, anchors)
    timing["calibration_sec"] = round(time.perf_counter() - t2, 3)
    timing["filter_and_adjust_sec"] = round(time.perf_counter() - t0, 3)
    return {
        "pred_df": pred_df,
        "age_curve_df": age_curve_df if age_curve_df is not None else pd.DataFrame(),
        "comp_df": comp_df if comp_df is not None and not comp_df.empty else pd.DataFrame(),
        "player_count": int(len(pred_df)),
    }, timing, None, None


def _run_ml_projection_pipeline(
    *,
    yearly_source,
    ml_lookback,
    ml_min_games,
    ...
    """Train + adjust ML projections; return status dict (never raises to caller)."""
    status = _empty_ml_pipeline_status()
    del ml_projection_style  # included in tuning signature at page layer
    t_total = time.perf_counter()
    try:
        ...
        if isinstance(base_pack, dict):
            ...
            ml_models = base_pack.get("ml_models", {})
            current_rows = base_pack.get("current_rows", pd.DataFrame())
            base_pred_df = base_pack.get("base_pred_df", pd.DataFrame())
            status["timing"] = dict(base_pack.get("timing") or {})
            ...
            current_rows = base_out["current_rows"]
            base_pred_df = base_out["base_pred_df"]
            status["timing"] = dict(base_out.get("timing") or {})
            base_pack = {
                "ml_training_df": ml_training_df,
                "ml_feature_cols": ml_feature_cols,
                "ml_models": ml_models,
            ...
            with st.spinner("Precomputing similarity comps and stabilization anchors…"):
                _finalize_ml_base_pack(base_pack, yearly_source, ml_lookback, year_pool_sig, refresh_token)
            status["timing"] = dict(base_pack.get("timing") or {})

        status["ml_training_df"] = ml_training_df
        status["ml_feature_cols"] = ml_feature_cols
        status["ml_models"] = ml_models
        status["current_rows"] = current_rows
        status["base_pred_df"] = base_pred_df

        if ml_training_df.empty or not ml_feature_cols:
            status["message"] = (
                "Not enough historical data to train models with these settings. "
                "Try lowering **Minimum Games in Lookback Window** or using a shorter **Lookback Window**."
            )
            status["fallback"] = "insufficient_training_data"
            return status

        if not ml_models:
            status["message"] = (
                "Models could not be trained (too few valid seasons per stat). "
                "Lower the minimum games filter or widen the lookback window."
            )
            status["fallback"] = "model_training_failed"
            return status

        if current_rows.empty or base_pred_df.empty:
            status["message"] = (
                "No active/recent players matched your scope. "
                "Increase **Projection Scope** or lower minimum games / AB filters."
            )
            status["fallback"] = "no_current_players"
            return status

        with st.spinner("Applying aging curves, regression, similar-player blend, and calibration…"):
            adj_out, adj_timing, fb, fb_msg = _run_ml_adjustment_and_calibration(
                yearly_source=yearly_source,
                ml_lookback=ml_lookback,
                ml_min_ab=ml_min_ab,
                base_pred_df=base_pred_df,
                ...
                k_neighbors=k_neighbors,
                refresh_token=refresh_token,
                base_pack=base_pack if isinstance(base_pack, dict) else None,
            )
        status["timing"].update(adj_timing or {})
        if fb:
            status["message"] = fb_msg
            status["fallback"] = fb
            return status

        status["pred_df"] = adj_out["pred_df"]
        status["age_curve_df"] = adj_out["age_curve_df"]
        status["comp_df"] = adj_out["comp_df"]
        status["player_count"] = adj_out["player_count"]
        status["base_pack"] = base_pack if isinstance(base_pack, dict) else None
        status["ok"] = True
        total_sec = time.perf_counter() - t_total
        status["runtime_sec"] = round(total_sec, 2)
        status["timing"]["total_sec"] = status["runtime_sec"]
        status["message"] = (
            f"Generated {status['player_count']:,} player projections "
            f"in {status['runtime_sec']:.1f} seconds."
        )
        if float(effective_comp_weight or 0) <= 0:
            status["fallback"] = "similar_players_disabled"
        return status
    except Exception as exc:
        status["message"] = f"ML predictions could not be generated: {exc}"
        status["fallback"] = "exception"
        status["runtime_sec"] = round(time.perf_counter() - t_total, 2)
        return status
...
def _ml_similarity_sim_cols(feature_cols):
    feature_cols = list(feature_cols or [])
    sim_cols = [c for c in _ML_SIMILARITY_PREFERRED_COLS if c in feature_cols]
    if len(sim_cols) < 5:
        sim_cols = feature_cols[:30]
    return sim_cols


def _build_ml_similarity_index(ml_training_df, feature_cols_tuple):
    """Reusable BallTree + target matrix for fast k-only similarity reruns."""
    feature_cols = list(feature_cols_tuple or [])
    if ml_training_df is None or ml_training_df.empty or not feature_cols:
        return None
    sim_cols = _ml_similarity_sim_cols(feature_cols)
    if not sim_cols:
        return None
    ...
    train_Z = ((train_X - means) / stds).to_numpy(dtype=np.float32)
    if len(train_Z) < 1:
        return None
    target_stats = [s for s in ML_TARGET_STATS if f"target_{s}" in ml_training_df.columns]
    target_mat = (
        ml_training_df[[f"target_{s}" for s in target_stats]]
    ...
    return {
        ...
        "train_Z": train_Z,
        "tree": tree,
        "train_ages": safe_numeric_series(ml_training_df, "age_entering_year", np.nan).to_numpy(),
...
def _query_ml_similarity_comps(sim_index, current_rows, k_neighbors=10, max_age_gap=3):
    """Query a pre-built similarity index (new k does not rebuild BallTree)."""
    if not sim_index or current_rows is None or current_rows.empty:
        return pd.DataFrame()
    target_stats = list(sim_index.get("target_stats") or [])
    target_mat = sim_index.get("target_mat")
    sim_cols = sim_index["sim_cols"]
    train_Z = sim_index["train_Z"]
    train_ages = sim_index["train_ages"]
    train_player_ids = sim_index["train_player_ids"]
    current_X = current_rows.reindex(columns=sim_cols).replace([np.inf, -np.inf], np.nan).fillna(0).astype(float)
    current_Z = ((current_X - sim_index["means"]) / sim_index["stds"]).to_numpy(dtype=np.float32)
    current_ages = safe_numeric_series(current_rows, "age_entering_year", np.nan).to_numpy()
    current_reset = current_rows.reset_index(drop=True)
    k_fetch = min(max(int(k_neighbors) * 5, int(k_neighbors) + 10), len(train_Z))
    if k_fetch < 1:
        return pd.DataFrame()
    tree = sim_index.get("tree")
    try:
        if tree is not None:
            dist_mat, idx_mat = tree.query(current_Z, k=k_fetch)
        else:
            dist_mat, idx_mat = None, None
    except Exception:
        dist_mat, idx_mat = None, None
    out_rows = []
    for i, row in current_reset.iterrows():
        if dist_mat is not None:
            row_dists = np.asarray(dist_mat[i]).ravel()
            row_idx = np.asarray(idx_mat[i]).ravel()
        else:
            diff = train_Z - current_Z[i]
            row_dists = np.sqrt(np.einsum("ij,ij->i", diff, diff))
            row_idx = np.arange(len(row_dists))
        ...
        for pos in order:
            idx = int(row_idx[pos]) if dist_mat is not None else int(pos)
            if idx < 0 or idx >= len(train_player_ids):
                continue
            comp_pid = str(train_player_ids[idx])
            if not comp_pid or comp_pid == target_pid or comp_pid in seen_comp_players:
                continue
            if pd.notna(age) and abs(train_ages[idx] - age) > max_age_gap:
                continue
            unique_nearest.append(idx)
            seen_comp_players.add(comp_pid)
            if len(unique_nearest) >= k_neighbors:
                break
        if not unique_nearest and pd.notna(age):
            for pos in order:
                idx = int(row_idx[pos]) if dist_mat is not None else int(pos)
                if idx < 0 or idx >= len(train_player_ids):
                    continue
        ...
        if not unique_nearest:
            continue
        out = {"playerID": row.get("playerID"), "Similar Player Sample": len(unique_nearest)}
        if target_mat is not None and target_stats:
            comp_vals = target_mat[unique_nearest]
            for j, stat in enumerate(target_stats):
                out[f"Similar {stat}"] = float(np.nanmean(comp_vals[:, j]))
        out_rows.append(out)
    return pd.DataFrame(out_rows)


@st.cache_data(show_spinner=False)
def build_similar_player_predictions(
    current_rows,
    ml_training_df,
    feature_cols_tuple,
    target_stats_tuple=tuple(ML_TARGET_STATS),
    ...
    del target_stats_tuple, refresh_token
    if current_rows.empty or ml_training_df.empty or not feature_cols_tuple:
        return pd.DataFrame()
    sim_index = _build_ml_similarity_index(ml_training_df, feature_cols_tuple)
    return _query_ml_similarity_comps(sim_index, current_rows, k_neighbors=k_neighbors, max_age_gap=max_age_gap)


def apply_advanced_projection_adjustments(
    pred_df,
    current_rows,
    ml_training_df,
    feature_cols,
...
def load_data(_hof_cache_key: float = 0.0):
    ...
    try:
        from hall_of_fame_data import attach_hof_flag, load_hall_of_fame_player_ids

        hof_ids = load_hall_of_fame_player_ids(BASE_DIR)
        people = attach_hof_flag(people, hof_ids)
        batting = attach_hof_flag(batting, hof_ids)
        yearly = attach_hof_flag(yearly, hof_ids)
    except ImportError:
        pass
...
def compute_team_aware_draft_room_fit(
    ...
    roster_hr = safe_numeric_series(roster, "proj_HR", 0).mean()
    roster_sb = safe_numeric_series(roster, "proj_SB", 0).mean()
    roster_ba = safe_numeric_series(roster, "proj_BA", 0).mean()
    roster_r = safe_numeric_series(roster, "proj_R", 0).mean()
    roster_rbi = safe_numeric_series(roster, "proj_RBI", 0).mean()

    league_hr = safe_numeric_series(out, "proj_HR", 0).median()
    league_sb = safe_numeric_series(out, "proj_SB", 0).median()
    league_ba = safe_numeric_series(out, "proj_BA", 0).median()
    league_r = safe_numeric_series(out, "proj_R", 0).median()
    league_rbi = safe_numeric_series(out, "proj_RBI", 0).median()
...
def build_lineup_assistant_scores(roster_stats, scoring_format="5x5 Roto", custom_weights=None):
    ...
    volume = normalize_series(safe_numeric_series(df, "AB", 0)) if "AB" in df.columns else pd.Series(0.5, index=df.index)
    rate_floor = normalize_series(safe_numeric_series(df, "OPS", np.nan).fillna(safe_numeric_series(df, "BA", 0)))
    ...
    *,
    ...
    if weakest_cat == "SB" or (weakest_cat and "SB" in out["weakest"] and sb_tot < 15):
        recs.append({
            "Type": "Draft target",
            "Label": "Need",
            "Detail": f"You only have **{sb_tot:.0f}** SB from this starting group. Look for middle infielders or outfielders with steady steals and a playable {rate_label}.",
        })
    if weakest_cat in ("AVG", "OBP") or (weakest_cat and weakest_cat in out["weakest"] and weakest_cat in ("AVG", "OBP")):
        ...
        recs.append({
            ...
            "Label": "Need",
            "Detail": f"Your starters’ {weakest_cat} is **{tr:.3f}**. Add contact hitters or bench the lowest-{rate_label} names in this lineup.",
        })
    if weakest_cat == "R" or (weakest_cat and "R" in out["weakest"] and r_tot < 40):
        recs.append({
            "Type": "Draft target",
            "Label": "Target profile",
            "Detail": f"Starters total only **{r_tot:.0f}** runs. Target table-setters and on-base bats who score, not only home-run hitters.",
        })
...
def build_simulated_draft_impact(player_name, team_name, lookup_df, name_col="fullName"):
    ...
    for label, actual_col, proj_col, is_rate in [
        ...
        impact_rows.append({
            "Category": label,
            "Current Projection": before,
            "Added Player": player,
            "New Projection": after,
            "Change": after - before,
        })
    impact_df = pd.DataFrame(impact_rows)
    after_detail_rows = _rows_for_player_names(after_players, lookup_df, name_col)
    roster_rows = []
    name_lookup = _projection_name_series(after_detail_rows, name_col) if not after_detail_rows.empty else pd.Series(dtype="object")
    for pname in after_players:
        m = after_detail_rows[name_lookup.eq(str(pname).strip())] if not after_detail_rows.empty else pd.DataFrame()
        r = m.iloc[0] if not m.empty else {}
        pos = r.get("Primary Position", r.get("primaryPos", "")) if hasattr(r, "get") else ""
        display_name = pname + (" (Simulated Next Pick)" if str(pname).strip() == player_name else "")
        roster_rows.append({
            "Position": pos,
            "Player": display_name,
        })
...
def draft_simulation_summary(player_name, impact_df):
    if impact_df is None or impact_df.empty:
        return f"Drafting {player_name} could not be summarized from the available projection columns."
    df = impact_df.copy()
    df["Change"] = pd.to_numeric(df["Change"], errors="coerce")
    pos = df[
        ((df["Category"].isin(["HR", "RBI", "R", "SB"])) & (df["Change"] > 0)) |
        ((df["Category"].isin(["AVG", "OPS"])) & (df["Change"] > 0.0005))
    ].copy()
    if pos.empty:
        return f"Adding {player_name} does not show a meaningful gain in the displayed hitting categories."
    priority = {"HR": 0, "RBI": 1, "R": 2, "SB": 3, "AVG": 4, "OPS": 5}
    pos["_priority"] = pos["Category"].map(priority).fillna(99)
    cats = pos.sort_values(["_priority", "Change"], ascending=[True, False])["Category"].head(5).tolist()
    pretty = {"R": "Runs", "AVG": "AVG", "OPS": "OPS"}
    cat_text = ", ".join(pretty.get(c, c) for c in cats)
    return f"Adding {player_name} improves your projected {cat_text}."
...
def _format_sim_value(category, value):
    return fmt_rate_4(value) if str(category) in {"AVG", "OPS"} else fmt_count_1(value)
...
def render_draft_simulation_result(result):
    ...
    if isinstance(impact_df, pd.DataFrame) and not impact_df.empty:
        ...
        for col in ["Current Projection", "Added Player", "New Projection", "Change"]:
            display_impact[col] = display_impact.apply(lambda r: _format_sim_value(r["Category"], r[col]), axis=1)
        st.caption("Simple projected category impact")
...
def build_draft_lab_player_pool(yearly_source, market_df, draft_window=3, fantasy_format="5x5 Roto", projection_style="Balanced"):
    ...
    pool = pool[(safe_numeric_series(pool, "G", 0) >= 30) & (safe_numeric_series(pool, "AB", 0) >= 75)].copy()
    ...
    pool["Trend Signal"] = normalize_series(
        safe_numeric_series(pool, "HR_trend", 0) * 0.35 +
        safe_numeric_series(pool, "RBI_trend", 0) * 0.25 +
        safe_numeric_series(pool, "SB_trend", 0) * 0.20 +
        safe_numeric_series(pool, "OPS_trend", 0) * 20
    )
    pool["Sleeper Score"] = normalize_series(
        normalize_series(safe_numeric_series(pool, "Capped Fantasy Edge", 0).clip(lower=0)) * 0.35 +
...
def _build_stabilized_anchors_from_recent(recent: pd.DataFrame) -> pd.DataFrame:
    """Shared stabilized anchor builder (used by per-set and full-pool caches)."""
...
@st.cache_data(show_spinner=False)
def build_ml_stabilized_anchor_pool(yearly_source, lookback, year_pool_sig, refresh_token=0):
    """All recently-active players' stabilization anchors (subset on tuning reruns)."""
    del refresh_token
    if yearly_source is None or yearly_source.empty:
        return pd.DataFrame()
    max_year = int(pd.to_numeric(yearly_source["yearID"], errors="coerce").max())
    ...
    if recent.empty:
        return pd.DataFrame()
    latest_year = recent.groupby("playerID", sort=False)["yearID"].max()
    active_ids = latest_year.index[latest_year >= max_year - 1]
    recent = recent[recent["playerID"].isin(active_ids)].sort_values(["playerID", "yearID"])
    return _build_stabilized_anchors_from_recent(recent)
...
def _weighted_rate(df, col):
    vals = safe_numeric_series(df, col, np.nan)
    weight_col = "proj_AB" if str(col).startswith("proj_") and "proj_AB" in df.columns else "AB"
    weights = safe_numeric_series(df, weight_col, 1).fillna(0)
...
def analyze_draft_lab_results(draft_df, yearly_source, *, context=None):
    from draft_lab_analysis import analyze_draft_lab_results as _analyze_core

    team_summary, strengths, pick_rows, gap_rows, _empty = _analyze_core(
        draft_df,
        yearly_source,
        context=context,
    )
    actual_summary = _draft_lab_actual_2026_summary(draft_df, yearly_source)
    return team_summary, strengths, pick_rows, gap_rows, actual_summary


def _draft_lab_actual_2026_summary(draft_df, yearly_source):
    if draft_df is None or draft_df.empty:
        return pd.DataFrame()
    actual_available = 2026 in set(safe_numeric_series(yearly_source, "yearID", np.nan).dropna().astype(int))
    ...
    for team, g in draft_actual.groupby("Fantasy Team"):
        actual_rows.append({
            "Fantasy Team": team,
            "Actual HR": safe_numeric_series(g, "HR_Actual", np.nan).sum(),
            "Actual RBI": safe_numeric_series(g, "RBI_Actual", np.nan).sum(),
            "Actual R": safe_numeric_series(g, "R_Actual", np.nan).sum(),
            "Actual SB": safe_numeric_series(g, "SB_Actual", np.nan).sum(),
            "Actual OPS": safe_numeric_series(g, "OPS_Actual", np.nan).mean(),
        })
...
def live_draft_push_analysis_to_session(room):
    ...
    try:
        ...
        st.session_state["_draft_lab_team_names"] = draft_lab_roster_team_options(list(room.get("teams") or []))
    except ImportError:
        pass
...
def _persist_live_draft_room(room, *, reason: str, rerun: bool = True) -> None:
    ...
    _no_rerun = reason in ("start_draft",)
    if _no_rerun:
        rerun = False
    try:
        from live_draft_start_progress import defer_cloud_autosave_during_start, is_live_draft_start_in_flight, mark_start_step

        if reason == "start_draft" or is_live_draft_start_in_flight(st.session_state):
            defer_cloud_autosave_during_start(st.session_state)
            mark_start_step(st.session_state, "persist_begin", shared_write_start_ts=time.time())
    except ImportError:
        mark_start_step = None  # type: ignore[assignment]
    ...
    try:
        ...
        shared_ok = False
        shared_err = str(exc)
        st.warning(f"Shared room sync failed: {exc}")
    ...
    try:
        if mark_start_step:
            mark_start_step(
                st.session_state,
                "shared_write_done",
                shared_write_end_ts=time.time(),
                shared_write_ok=shared_ok,
                shared_write_error=shared_err or None,
            )
    except Exception:
        pass
    ...
    try:
        ...
        mark_start_step(
            st.session_state,
            "persist_complete",
            timer_deadline_set=bool(isinstance(room, dict) and room.get("timer_deadline")),
        )
    except ImportError:
        pass
    if rerun:
...
def compact_player_action_center(
    ...
    sim_result = st.session_state.get(f"{key}_draft_simulation_result")
    if isinstance(sim_result, dict):
        st.markdown("##### Simulate Draft Pick")
        render_draft_simulation_result(sim_result)
        if st.button("Clear simulation", key=f"{key}_clear_draft_simulation"):
...
@st.cache_data(show_spinner=False)
def _cached_awards_players_df(_awards_cache_key: float):
    """Cache AwardsPlayers.csv; bust cache when file mtime changes."""
    try:
        from awards_players_data import load_awards_players_df

        return load_awards_players_df(BASE_DIR)
    except ImportError:
        return pd.DataFrame()

try:
    from awards_players_data import awards_file_cache_key

    AWARDS_CACHE_KEY = awards_file_cache_key(BASE_DIR)
except ImportError:
    AWARDS_CACHE_KEY = 0.0

AWARDS_PLAYERS_DF = _cached_awards_players_df(AWARDS_CACHE_KEY)
...
def record_workflow_comparison_group(labels):
    """Store a compact recent comparison/trend group in the sidebar history."""
    names = []
    for label in labels or []:
        name = fullname_base_from_label(label)
    ...
    kept = [
        ...
        and len(p) >= 2
        and tuple(sorted(str(x).strip() for x in p if str(x).strip())) != sig
    ]
    kept.append(names)
    st.session_state["workflow_recent_compare_pairs"] = kept[-wf_sb.RECENT_COMPARE_CAP:]
...
LEGACY_WIDGET_KEY_MIGRATIONS = {
    "historical_year_range_filter": "hist_year",
    "historical_batting_hand_filter": "hist_bats",
    "historical_team_filter": "hist_team",
    "historical_position_filter": "hist_pos",
    "historical_position_filter_mode": "hist_position_filter_mode",
    "historical_sort_stat_filter": "hist_sort_stat",
    "historical_sort_order_filter": "hist_sort_order",
    "historical_combine_split_seasons_filter": "hist_combine_split_seasons",
    "career_year_range_filter": "career_year",
    "career_batting_hand_filter": "career_bats",
    "career_team_filter": "career_team",
    "career_position_filter": "career_pos",
    "career_position_filter_mode": "career_position_filter_mode",
    "career_sort_stat_filter": "career_sort",
    "career_by_team_toggle_filter": "career_by_team_toggle",
    "leaders_year_range_filter": "leaders_year",
    "leaders_top_n_filter": "leaders_top_n",
...
PAGE_STATE_DEBUG_PREFIXES = {
    "Historical Explorer": ("historical_", "hist_"),
    "Career Totals": ("career_",),
    "Leaderboards": ("leaders_",),
    "Comparison Tool": ("compare_", "sig_"),
    "Trend Value": ("trend_",),
    "Valuation": ("value_",),
...
def migrate_legacy_widget_keys():
    ...
    import copy
    for new_key, old_key in LEGACY_WIDGET_KEY_MIGRATIONS.items():
        if old_key in st.session_state and new_key not in st.session_state:
            st.session_state[new_key] = copy.deepcopy(st.session_state[old_key])
    if "_active_page_selector" in st.session_state and MAIN_SIDEBAR_PAGE_KEY not in st.session_state:
        old_page = normalize_page_key(st.session_state["_active_page_selector"])
        if old_page in _PAGE_OPTION_SET:
...
def validate_number_state(key, default, min_value=None, max_value=None, as_int=True):
    if key not in st.session_state:
        ...
        try:
            val = int(st.session_state[key]) if as_int else float(st.session_state[key])
...
if active_page == "Career Totals":
    ...
    if hof_case_mode:
        ...
        if career_player_options:
            validate_state_option(CAREER_HOF_CASE_TARGET_KEY, career_player_options, default_target)
        st.selectbox(
            "Target player",
            career_player_options,
    ...
    try:
        ...
        cache_page_context(
            st.session_state,
            "Career Totals",
            {"career_snapshot": career_snap, "metrics": [str(sort_stat_career)]},
        )
    except Exception:
        pass

    _career_xfer_df = career_totals.copy()
...
if active_page == "Trend Value":
    ...
    def _trend_insight_player_changed():
        # Persist the new selection to page_filter_state immediately so that the
        # next rerun's apply_baseball_disk_state restore sees the updated value.
        save_page_state("Trend Value")
...
if active_page == "Fantasy Sleepers & Busts":
    ...
    fantasy_age_range = st.session_state.get("fantasy_market_age_range", (18, 45))
    ...
    if fantasy_df.empty:
        ...
        fantasy_view_mode = st.session_state.get("fantasy_edge_scatter_view_mode", "Focused View")
        fantasy_trendline_type = st.session_state.get("fantasy_market_edge_trendline_type", "Auto Best Fit")
        ...
        def _sleepers_player_selection_changed():
            try:
                ...
                pass
            # Persist the new selection to page_filter_state immediately.
            save_page_state("Fantasy Sleepers & Busts")
...
if active_page == "Draft Assistant Simulator":
    ...
    if market_df.empty:
        ...
        if "Market Rank" in available.columns:
            mkt_left = available[pd.to_numeric(available["Market Rank"], errors="coerce").notna()].copy()
            mkt_left = mkt_left.sort_values("Market Rank", ascending=True).head(12)
        else:
            mkt_left = available.sort_values("Expected Fantasy Value", ascending=False).head(12)
        mkt_cols = ["fullName", "Team", "Primary Position", "Market Rank", "Model Rank", "Fantasy Edge", "Expected Fantasy Value"]
        mkt_display = mkt_left[[c for c in mkt_cols if c in mkt_left.columns]].rename(columns={"fullName": "Player"})
        if not mkt_display.empty:
            render_output_table(
                format_fantasy_table(clean_ui_columns(mkt_display)),
                key="draft_assistant_top_market_left",
                file_name="draft_assistant_top_market_left.csv",
                display_rows=12,
                style_cols=["Fantasy Edge"],
            )
        else:
            st.info("No available players with market rank data. Load FantasyPros CSVs or check Draft Room picks.")
        ...
        with st.expander("Position scarcity & roster category heatmap", expanded=False):
            ...
            if roster_df.empty:
                ...
                for label, col in cat_defs.items():
                    roster_val = safe_numeric_series(roster_df, col, 0).sum() if col != "proj_BA" else safe_numeric_series(roster_df, col, np.nan).mean()
                    pool_values = safe_numeric_series(draft_df, col, 0)
                    pool_avg = pool_values.mean()
                    pool_std = pool_values.std()
...
if active_page == "Draft Simulation Test Mode":
    ...
    with lc3:
        ...
        lab_projection_style = st.selectbox("Projection Style", list(PROJECTION_STYLE_OPTIONS),
                                            ...
                                            on_change=_draft_sim_setting_changed)
    with lc4:
        _lab_picks_default = int(st.session_state.get("draft_lab_picks_per_team") or 15)
        validate_number_state("draft_lab_picks_per_team", _lab_picks_default, min_value=1, max_value=25)
    ...
    if run_lab:
        ...
        with st.spinner("Building draft pool and simulating 60 picks..."):
            ...
            sim_ctx = {"config": {"slots": {"C": 1, "1B": 1, "2B": 1, "3B": 1, "SS": 1, "OF": 3, "DH": 1, "BN": 5}}, "teams": sim_teams, "pool": lab_pool}
            lab_team_summary, lab_strengths, lab_pick_analysis, lab_gaps, lab_actual_summary = analyze_draft_lab_results(
                lab_draft, yearly_df, context=sim_ctx
            )
            lab_trades = suggest_draft_lab_trades(lab_draft, lab_team_summary, max_suggestions=12)
            try:
                from draft_lab_analysis import draft_lab_roster_team_options

                st.session_state["_draft_lab_team_names"] = draft_lab_roster_team_options(sim_teams)
            except ImportError:
                pass
            st.session_state["draft_lab_results"] = {
                "pool": lab_pool,
    ...
    lab_analysis_ctx = lab_state.get("analysis_context") if isinstance(lab_state.get("analysis_context"), dict) else {}
    try:
        from draft_lab_analysis import (
            FANTASY_EDGE_HELP,
            analysis_context_from_session,
        ...
        analysis_context_from_session = lambda _s, _l=None: {}
    ...
    if is_dataframe_empty(lab_draft):
        ...
        if lab_analysis_ctx:
            _ctx.update(lab_analysis_ctx)
        _lab_teams = list(_ctx.get("teams") or [])
        if not _lab_teams and not is_dataframe_empty(lab_draft) and "Fantasy Team" in lab_draft.columns:
            _lab_teams = sorted(lab_draft["Fantasy Team"].dropna().astype(str).unique().tolist())

        draft_show_cols = draft_lab_board_display_columns() if callable(draft_lab_board_display_columns) else []
        if not draft_show_cols:
            draft_show_cols = [
        ...
        with tabs[3]:
            ...
            if developer_mode_enabled() and not is_dataframe_empty(lab_draft) and "fullName" in lab_draft.columns:
                try:
                    from draft_lab_analysis import decision_score_debug_row

                    with st.expander("Decision score component debug", expanded=False):
                        _dbg_opts = [""] + lab_draft["fullName"].astype(str).tolist()
                        _dbg_player = st.selectbox("Inspect player", _dbg_opts, key="draft_lab_decision_debug_player")
                        if _dbg_player:
                            row = lab_draft[lab_draft["fullName"].astype(str) == _dbg_player].iloc[0]
                            st.json(decision_score_debug_row(row))
                except ImportError:
                    pass
...
if active_page == "Live Draft Room":
    ...
    if st.session_state.pop("_start_live_draft_pending", False):
        ...
        try:
            ...
            begin_live_draft_start(st.session_state, mode=_start_mode)
        ...
        if not _skip_pool_build:
            ...
            if pool_live.empty:
                ...
                _start_handler_err = (
                    f"Not enough players in the pool ({len(pool_live):,}) for {total_picks:,} total picks."
                )
                record_start_live_draft_diagnostics(st.session_state, start_live_draft_error=_start_handler_err)
                st.error(_start_handler_err)
            else:
                ...
                try:
                    if mark_start_step:
                        mark_start_step(
                            st.session_state,
                            "room_initialized",
                            live_room_created=True,
                            room_created_ts=_time_mod.time(),
                            room_initialized_ts=_time_mod.time(),
                        )
                except Exception:
                    pass
                ...
                if not promote_ok and _from_simulator and sim_pick_count > 0:
                    _start_handler_err = promote.get("error") or "Could not promote simulator board into live draft."
                    record_start_live_draft_diagnostics(st.session_state, start_live_draft_error=_start_handler_err)
                    st.error(_start_handler_err)
                else:
    ...
    if market_df_live is not None:
        try:
            ...
            if not is_live_draft_start_in_flight(st.session_state):
    ...
    if room is None:
        ...
        try:
            ...
            share_code = resolve_shared_room_code(st.session_state)
        except Exception:
            share_code = ""
            is_plausible_share_code = lambda c: bool(c)  # type: ignore[assignment, misc]
        if _multiplayer_draft or share_code:
            if share_code and is_plausible_share_code(share_code):
                room_label = f"Draft Room Code **{share_code}**"
            elif _multiplayer_draft:
                room_label = "Multiplayer draft (code missing)"
            else:
        ...
        if team_roster.empty:
            ...
            if lineup_trade_roster_stats.empty:
                ...
                if not other_trade_teams:
                    ...
                    ensure_select_in_options("lineup_trade_other_team", other_trade_teams, other_trade_teams[0])
                    other_team_trade = st.selectbox("Other Team", other_trade_teams, key="lineup_trade_other_team")
...
if active_page == "ML Predictions":
    ...
    if not SKLEARN_AVAILABLE:
        ...
        st.button(
            ...
            type="primary",
            key="ml_predictions_refresh_button",
            on_click=_on_ml_predictions_refresh_click,
        )
        ...
        if st.session_state.get("ml_predictions_have_run", False):
            if status:
                ...
                st4.caption("Notes")
                _fb = status.get("fallback")
                st4.write(
                    "Similar-player blend off"
                    if _fb == "similar_players_disabled"
