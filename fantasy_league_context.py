...
def _canonical_live_created_from(
    *,
    context: dict[str, Any] | None = None,
    shared_doc: dict[str, Any] | None = None,
    archive_entry: dict[str, Any] | None = None,
    ...
    if isinstance(archive_entry, dict):
        ...
        # Live Draft → Create Shared League stamps this on the archive row.
        if bool(archive_entry.get("shared_league_created")):
            return True
...
# Prefer non-underscore keys so Streamlit session_state does not treat them as internal.
ORIGIN_REPAIR_DECISIONS_KEY = "library_origin_repair_decisions"
ORIGIN_MIGRATION_TRACE_KEY = "library_origin_migration_trace"
# Legacy keys (still read for one release).
_LEGACY_ORIGIN_REPAIR_DECISIONS_KEY = "_draft_origin_repair_decisions"
...
def get_origin_repair_decisions(session: dict[str, Any] | None) -> list[dict[str, Any]]:
    """Return per-league decisions from session key or migration trace (never lose UI payload)."""
    if not isinstance(session, dict):
        return []
    for key in (ORIGIN_REPAIR_DECISIONS_KEY, _LEGACY_ORIGIN_REPAIR_DECISIONS_KEY):
        rows = session.get(key)
        if isinstance(rows, list) and rows:
            return [dict(row) for row in rows if isinstance(row, dict)]
    for key in (ORIGIN_MIGRATION_TRACE_KEY, "_origin_migration_trace", "_library_repair_last_trace"):
        trace = session.get(key)
        if not isinstance(trace, dict):
            continue
        rows = trace.get("decisions")
        if isinstance(rows, list) and rows:
            return [dict(row) for row in rows if isinstance(row, dict)]
    return []


def evaluate_origin_decisions_for_visible_archives(session: dict[str, Any]) -> list[dict[str, Any]]:
    """Always record resolve decisions for visible library cards (even when gated repair skips)."""
    try:
        from draft_archive_visibility import list_visible_draft_archives
    except ImportError:
        from draft_archive_state import list_draft_archives as list_visible_draft_archives
    from fantasy_league_identity import resolve_canonical_league_id

    prior = get_origin_repair_decisions(session)
    ...
    try:
        for entry in list_visible_draft_archives(session):
            ...
            decision = {
                ...
                "creation_origin_before": read_immutable_creation_origin(
                    context=ctx if isinstance(ctx, dict) else None,
                    shared_doc=shared_doc if isinstance(shared_doc, dict) else None,
                    archive_entry=entry,
                ),
                ...
                "context_loaded": isinstance(ctx, dict),
                "shared_league_created_flag": bool(entry.get("shared_league_created")),
                "shared_created_from": str((shared_doc or {}).get("created_from") or "").strip()
                if isinstance(shared_doc, dict)
                else "",
                "context_created_from": str(((ctx or {}).get("metadata") or {}).get("created_from") or "").strip()
                if isinstance(ctx, dict)
                else "",
                "source": "evaluate_visible_archives",
                "origin_tokens_found": list(evidence.get("origin_tokens_found") or []),
            }
            ...
            if str(entry.get("draft_type") or "").strip() != expected:
                ...
                decision["archive_type_after"] = expected
    except Exception:
        # Never wipe a successful prior payload if re-evaluation fails mid-render.
        if prior and not decisions:
            session[ORIGIN_REPAIR_DECISIONS_KEY] = prior
            return prior
        raise

    session[ORIGIN_REPAIR_DECISIONS_KEY] = decisions
    session.pop(_LEGACY_ORIGIN_REPAIR_DECISIONS_KEY, None)
    return decisions
...
def resolve_archive_draft_type_with_reason(
    *,
    ...
    # Live Draft Shared Leagues keep created_from=live_draft even when a bad
    # repair stamped creation_origin/source as import. Prefer the creation event.
    if _canonical_live_created_from(context=context, shared_doc=shared_doc, archive_entry=archive_entry):
        if creation_origin == CREATION_ORIGIN_VALIDATED_IMPORT:
            return DRAFT_TYPE_LIVE, "poisoned_import_origin_overridden_by_live_created_from", evidence
        return DRAFT_TYPE_LIVE, "canonical_created_from_live_draft", evidence
    if creation_origin == CREATION_ORIGIN_VALIDATED_IMPORT:
...
def repair_canonical_shared_doc_origin_in_place(
    ...
    *,
    ...
    existing_origin = read_immutable_creation_origin(context=context, shared_doc=shared_doc)
    live_created = _canonical_live_created_from(context=context, shared_doc=shared_doc)
    if existing_origin == CREATION_ORIGIN_VALIDATED_IMPORT and not live_created:
        return None
    ...
    if selected_reason not in {
        ...
        return None
    already_live = _shared_origin_fields_are_live(shared_doc) and existing_origin == CREATION_ORIGIN_LIVE_DRAFT_ROOM
    if already_live:
        return shared_doc
    try:
    ...
    repaired_meta["creation_origin"] = CREATION_ORIGIN_LIVE_DRAFT_ROOM
...
def apply_draft_origin_to_context(
    ...
    *,
    ...
    if draft_type == DRAFT_TYPE_LIVE:
        ...
        # Force-correct poisoned immutable origin left by earlier repair/backfill.
        if (
            str(meta.get("creation_origin") or out.get("creation_origin") or "").strip()
            ...
            and (
                ...
                or _canonical_live_created_from(
                    context=out, shared_doc=shared_doc, archive_entry=archive_entry
                )
            )
        ):
            meta["creation_origin"] = CREATION_ORIGIN_LIVE_DRAFT_ROOM
            out["creation_origin"] = CREATION_ORIGIN_LIVE_DRAFT_ROOM
        else:
            meta = stamp_immutable_creation_origin(meta, CREATION_ORIGIN_LIVE_DRAFT_ROOM)
            if not str(out.get("creation_origin") or "").strip():
                out["creation_origin"] = str(meta.get("creation_origin") or "") or CREATION_ORIGIN_LIVE_DRAFT_ROOM
...
def repair_archive_draft_type_for_entry(
    ...
    *,
    ...
    if current != expected:
        ...
        changed = True
    if expected == DRAFT_TYPE_LIVE:
        origin = read_immutable_creation_origin(
            context=ctx if isinstance(ctx, dict) else None,
            shared_doc=shared_doc if isinstance(shared_doc, dict) else None,
            archive_entry=repaired,
        )
        if origin == CREATION_ORIGIN_VALIDATED_IMPORT and _canonical_live_created_from(
            context=ctx if isinstance(ctx, dict) else None,
            shared_doc=shared_doc if isinstance(shared_doc, dict) else None,
            archive_entry=repaired,
        ):
            repaired["creation_origin"] = CREATION_ORIGIN_LIVE_DRAFT_ROOM
            changed = True
        elif not str(repaired.get("creation_origin") or "").strip():
            repaired["creation_origin"] = CREATION_ORIGIN_LIVE_DRAFT_ROOM
            changed = True
    if not changed:
        return entry
