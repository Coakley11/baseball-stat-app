...
def render_locked_weekly_dashboard(
    ...
    *,
    ...
    if not isinstance(scoring_record, dict) or not scoring_record.get("baseline_created_at"):
        if is_legacy_locked_lineup({"status": "locked"}, scoring_record):
            st.caption("Legacy locked lineup — weekly scoring baseline was not captured for this week.")
        return
    ...
    if bench:
        st.markdown("<h4>Bench — does not count</h4>", unsafe_allow_html=True)
        ...
        for pkey, meta in bench.items():
            ...
            bench_cards.append(
                ...
                f"<div class='fl-wk-role'>Bench</div>"
                f"{_stat_line_html(cats, display, points=pres.get('points_total'))}</div>"
            )
        st.markdown(f"<div class='fl-wk-grid'>{''.join(bench_cards)}</div>", unsafe_allow_html=True)

    team_totals = (scoring_record.get("team_totals") or {}).get("totals") or {}
    if team_totals:
        st.markdown(
    ...
    st.markdown('<p class="fl-wk-muted">Bench statistics are shown for reference only and do not count toward your team score.</p>', unsafe_allow_html=True)
